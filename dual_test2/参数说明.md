...
```json
{
  ...
  "sfx": false,
  "dual_version": true,
  "created": "2026-04-27 21:50:45.391078"
}
```
...
| `resolution` | `1080x1920` | 输出分辨率，如 `1920x1080`(横屏) / `1080x1920`(竖屏) |
| `fps` | `30` | 帧率，默认 30 |
| `voice` | `Xiaoxiao` | 默认AI配音音色 |
...
| `subtitle_style` | `tiktok` | 字幕样式预设，见下表 |
| `subtitle_animation` | `none` | 字幕动画: `none`(无) / `slide_up`(上滑进入) / `fade_in`(淡入) |
| `transition` | `pixelize` | 场景转场效果，见下表 |
...
| `dual_version` | `True` | 是否同时生成横竖双版本，`true`(开启) / `false`(关闭) |
...
```bash
python3 video_generator_pro.py -p dual_test2
```
...
```bash
python3 video_generator_pro.py -p dual_test2     --scene-fade 0.3     --subtitle --subtitle-animation slide_up     --transition pixelize     --watermark logo.png     --watermark-position bottom-right     --sfx
```

---
*生成时间: 2026-04-27 21:50:45*

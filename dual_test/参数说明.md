...
```json
{
  ...
  "sfx": false,
  "dual_version": false,
  "created": "2026-04-27 21:44:05.411441"
}
```
...
| `dual_version` | `False` | 是否同时生成横竖双版本，`true`(开启) / `false`(关闭) |
...
```bash
python3 video_generator_pro.py -p dual_test
```
...
```bash
python3 video_generator_pro.py -p dual_test     --scene-fade 0.3     --subtitle --subtitle-animation slide_up     --transition pixelize     --watermark logo.png     --watermark-position bottom-right     --sfx
```

---
*生成时间: 2026-04-27 21:44:05*
